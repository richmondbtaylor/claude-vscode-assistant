...
class ClawdBot:
    ...
    def log_activity(self, action, details):
        ...
        print(log_entry.strip().encode('ascii', 'replace').decode())
    ...
    def process_linkedin_post(self, message):
        ...
        for i, url in enumerate(new_urls):
            try:
                ...
                safe_content = post_data['content'][:200].encode('ascii', 'replace').decode()
                safe_comment = comment.encode('ascii', 'replace').decode()
                print(f"\n{'='*60}")
                print(f"POST: {safe_content}...")
                print(f"\nGENERATED COMMENT: {safe_comment}")
                print(f"{'='*60}")
                ...
                self.log_activity("ERROR", f"Error processing post {url}: {safe_err}")
                self._mark_url_processed(url)
                continue
    ...
    def process_feed_posts(self, count=10):
        ...
        for i, url in enumerate(new_urls):
            try:
                ...
                safe_err = str(e).encode('ascii', 'replace').decode()
                self.log_activity("ERROR", f"Error processing feed post {url}: {safe_err}")
                self._mark_url_processed(url)
                continue
...
def main():
    ...
    default_group = os.environ.get('WHATSAPP_GROUP', 'LinkedIn B2B Creators')
    group_name = default_group
    print(f"\nUsing WhatsApp group: {group_name}")
