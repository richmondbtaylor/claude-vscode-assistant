"""
SQLite-backed deduplication store for Conquer.io Social Prospecting Agent.
...
def save_analysis(post_id: str, platform: str, author: str, intent_score: int,
                  pain_category: str, should_contact: bool, company_name: str = "") -> None:
    with _connect() as conn:
        conn.execute("""
            INSERT OR REPLACE INTO analyzed_posts
                (id, platform, analyzed_at, intent_score, pain_category, should_contact, author, company_name)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (post_id, platform, datetime.utcnow().isoformat(),
              intent_score, pain_category, int(should_contact),
              author or "", company_name or ""))
        conn.commit()


def get_author_platforms(author: str, exclude_platform: str) -> list[str]:
    """Return list of OTHER platforms where this author has previously appeared."""
