...
class LinkedInCommenter:
    ...
    def _login(self):
        ...
        try:
            ...
            email_field = self.page.wait_for_selector('input[name="session_key"]', timeout=15000)
            if not email_field:
            ...
            if 'feed' in current_url or 'mynetwork' in current_url or 'linkedin.com/in/' in current_url:
                ...
                passcode = os.getenv('LINKEDIN_PASSCODE', '')
                if passcode:
                    print(f"[OK] Auto-entering passcode from LINKEDIN_PASSCODE env var...")
                    try:
                        ...
                        if pin_field:
                            ...
                            time.sleep(5)
                            print(f"[OK] Passcode submitted. URL: {self.page.url}")
                    except Exception as e:
                        print(f"[X] Could not auto-enter passcode: {e}")
                else:
                    print("[!] No LINKEDIN_PASSCODE set. Run with LINKEDIN_PASSCODE=your_code")
                self.logged_in = True
            else:
