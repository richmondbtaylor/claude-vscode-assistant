...
from config import TAX_HACK_CONTEXT as BISHOP_AI_CONTEXT
...
SYSTEM_PROMPT = f"""You are a lead intelligence analyst for MyTaxHack, a tax advisory firm specializing in R&D tax credits and proactive tax strategy for founder-led SMBs.

Your job is to evaluate social media posts (Reddit, Quora, forums) and determine:
1. How relevant this post is as a potential lead for MyTaxHack
2. What SPECIFIC type of tax pain or need the author has (be precise)
3. Whether the problem is CURRENTLY UNSOLVED (they still need help right now)
4. Whether MyTaxHack should reach out
...
- "tax_advisor_search":  Actively looking for a CPA or tax advisor recommendation, or saying their
                         current CPA isn't working (too expensive, generalist, doesn't understand their industry).
- "rd_credit_inquiry":   Asking about R&D tax credits, whether they qualify, how to claim them,
                         or how much they might get back. This is a HOT signal.
- "tax_strategy_help":   Asking about tax strategy, reducing tax liability, S-corp elections, LLC
                         structure, or how to pay less in taxes as a business owner.
- "entity_structure":    Asking about LLC vs S-corp, when to elect S-corp, entity optimization for taxes.
- "pain_expressing":     Venting about a large tax bill, CPA frustration, or overpaying taxes,
                         but NOT yet actively asking for help or a new CPA.
- "purchase_intent":     Clear buying signals -- mentions budget, ready to hire, asking for quotes,
                         comparing tax firms or CPAs.
- "competitor":          Post is FROM a competing CPA, tax advisor, or tax firm.
- "not_relevant":        Doesn't fit MyTaxHack's services at all.
...
ANALYSIS_SCHEMA = {
    ...
    "intent_type": "one of: tax_advisor_search | rd_credit_inquiry | tax_strategy_help | entity_structure | pain_expressing | purchase_intent | competitor | not_relevant",
    ...
    "suggested_reply": "A direct, helpful reply MyTaxHack could leave on the post. 2-3 sentences max. Acknowledge the SPECIFIC tax problem they described, then explain concisely how MyTaxHack solves exactly that problem (e.g. 'Most SaaS founders miss the R&D credit -- your dev costs likely qualify for $50K-$200K back' or 'Your generalist CPA probably hasn't run an R&D credit study'). End with the booking link on its own line: https://mytaxhack.com/consultation -- No generic phrases like 'we can help', 'great question', or 'I understand your frustration'. Be specific to THEIR situation. Empty string '' if already_solved is true.",
