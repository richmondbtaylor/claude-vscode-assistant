...
def run_web_cycle() -> None:
    print(f"\n{'='*60}")
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] WEB POLL CYCLE (Brave Search / LinkedIn / Quora / Twitter / Web)")
    print(f"{'='*60}")
    try:
    ...
    # Make, OpenAI Community, Zapier Community — Discourse scrapers
    print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] COMMUNITY FORUMS (Make / OpenAI / Zapier)")
    try:
        for post in community_monitor.poll():
            process_post(post)
    except Exception as e:
        print(f"[main] Community forums cycle error: {e}")

    # LinkedIn — dedicated DuckDuckGo site:linkedin.com search pass
    print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] LINKEDIN")
    try:
    ...
    # Facebook Groups -- Apify scraper
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] FACEBOOK GROUPS")
    try:
...
def main():
    ...
    if args.once:
        ...
        if run_web:
            run_web_cycle()
        notifier.flush_slack_buffer()  # Single Slack update at end of full run
