...
Slack sends one message per run — all leads are queued and flushed together
at the end of the full run via flush_slack_buffer().
"""
...
def log_to_sheets(post: RawPost, analysis: PostAnalysis, cross_platform: str = "") -> bool:
    ...
    try:
        ...
        _platform_labels = {
            ...
            "zapier_community": "Zapier Community",
...
def send_slack_alert(post: RawPost, analysis: PostAnalysis) -> bool:
    """
    Queue a lead for Slack. All leads held until flush_slack_buffer() fires
    once at end of the full run — one single Slack message per run.
...
def flush_slack_buffer() -> None:
    """Send all queued leads as one Slack message. Called once at end of full run."""
    webhook_url = os.environ.get("SLACK_WEBHOOK_URL", "")
